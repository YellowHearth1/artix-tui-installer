...
[🇺🇦 Українська](README.md) · [🇬🇧 English](README.en.md) · **🇲🇽/🇪🇸 Español** | [ich0x](https://github.com/ich0x)

### Un instalador TUI de Artix Linux — dinit, LUKS, reversión de btrfs, Wayland. Sin systemd.
...
<img alt="idiomas de la interfaz" src="https://img.shields.io/github/directory-file-count/YellowHearth1/artix-tui-installer/installer%2Fi18n?type=file&label=idiomas&style=for-the-badge&color=FFD700">
...
<img src="screenshots/es/14-summary.png" alt="El instalador en acción — pantalla de revisión e instalación" width="900">
...
Mapa del código, cambios comunes, compilación y pruebas.

**Un instalador de terminal para un spin personalizado
...
> ### ⚠️ Experimental — úsalo bajo tu propio riesgo
>
> Este **no es un instalador listo para producción**. El código lo escribe
> Claude — la nota de arriba lo dice sin rodeos. Mi parte es otra: decido qué
> debe ser esto y cómo debe comportarse, ejecuto las compilaciones en máquinas
> virtuales, detecto lo que se rompe y lo devuelvo al taller hasta que funcione
...
>
> - todo se prueba **únicamente en una máquina virtual** (QEMU/UEFI, distintos
>   tipos de medios). Este instalador **nunca** se ha ejecutado en hardware
>   físico;
> - el arranque dual se ha probado **solo con Artix junto a Artix**. **Nunca se
>   ha instalado Windows al lado.** Detectar Windows, reutilizar su partición
>   EFI, conservar sus particiones y generar su entrada de menú están escritos y
>   cubiertos por pruebas, pero confirmados en un sistema real exactamente cero
>   veces. Si tienes Windows, serás el primero en averiguarlo.
>
> **Este instalador formatea discos.** Un error aquí cuesta datos.
>
> - Pruébalo primero en una **máquina virtual** y confirma que tu escenario se
>   comporta como esperas.
> - **Haz una copia de seguridad** de todo lo que no estés dispuesto a perder, y
>   comprueba que realmente se puede restaurar.
> - En un equipo con otro sistema operativo, lee la pantalla de confirmación con
>   **atención**: enumera cada partición que será destruida.
> - ¿Algo salió mal? [Abre un issue](https://github.com/YellowHearth1/artix-tui-installer/issues).
...
>
> Las traducciones son bienvenidas y son la forma más fácil de contribuir: no
> hace falta saber Rust. El español ([ich0x](https://github.com/ich0x)) llegó
> exactamente así.
>
> **1. Copia el archivo de referencia.** El ucraniano es la referencia de la que
> parten todas las cadenas:
>
> ```sh
> cp installer/i18n/uk.toml installer/i18n/XX.toml   # XX = código de idioma, p. ej. de, pl, fr
> ```
>
> **2. Traduce los valores, nunca las claves.** La clave de la izquierda
> (`disk.warn`) es un identificador y debe ser idéntica en todos los archivos;
> solo se traduce el texto entre comillas. Los marcadores como `{}` y
> `@@NAME@@ ` deben conservarse.
>
> **3. Añade el nombre del idioma a TODOS los archivos de idioma.** Cada
> `*.toml` — incluido el tuyo — necesita una línea en `[lang]`:
>
> ```toml
> [lang]
> en = "English"
> uk = "Ukrainian (Українська)"
> de = "German (Deutsch) — traducción de tu-usuario"
> ```
>
> **4. Conéctalo en el código**: cuatro lugares pequeños — `Lang`
> (`installer/src/i18n.rs`), un cargador junto a `uk()`/`es()`, una rama en
> `t()`, y `OPTIONS` en `installer/src/screens/language.rs`, donde también se
> definen la locale y la distribución de teclado.
>
> **5. Compruébalo.** Las pruebas detectan una clave faltante, una cadena vacía,
> un idioma que nadie puede seleccionar y texto que desborda una pantalla de
> 80x24:
>
> ```sh
...
> ```
>
> Traducir el README (`README.XX.md`) es opcional y muy bienvenido. **Solo
> espacios, sin tabulaciones**: dentro de los bloques de código un tabulador se
> expande a ocho columnas y destroza los diagramas ASCII de las pantallas.
...
- **🌐 Interfaz multilingüe** — ucraniano, inglés y español; el idioma se elige en la
  primera pantalla.
...
- **🧨 Confirmación de borrado** — el último paso antes del formateo: un modal dedicado muestra con exactitud
  ...
  el disco equivocado.
- **🌍 La lista de mirrors sigue siendo legible y editable a mano.** El archivo
  reconstruido está **agrupado por país** — cada bloque lleva su encabezado de
  país, los países van ordenados por su mirror más rápido, y dentro de cada
  bloque los mirrors van por tiempo de respuesta. ¿Te mudaste a otro país?
  Comenta un bloque, descomenta otro. Eso es todo.
- **💤 Los mirrors que no pasan la comprobación se COMENTAN, no se eliminan** —
  junto con sus países, al final del archivo. El que hoy está caído puede ser el
  más cercano mañana, o después de una mudanza, así que se queda donde puedas
  encontrarlo.
...
![La pantalla de disco y particiones, en vivo](screenshots/es/09-disk.png)
...
│  ○  …     │  │  Sistema de archivos ‹ ext4 › btrfs  │    │
│           │  │            ◂ Atrás    Siguiente ▸    │    │
│           │  │                                      │    │
│           │  └──────────────────────────────────────┘    │
│           ├──────────────────────────────────────────────┤
│           │  ↑/↓ mover · ←/→ cambiar · Enter sigue       │
└───────────┴──────────────────────────────────────────────┘
```
...
Traducción al español (es-419, latinoamericano) — **[ich0x](https://github.com/ich0x)**:
archivo de idioma completo y documentación traducida. ¡Gracias!
