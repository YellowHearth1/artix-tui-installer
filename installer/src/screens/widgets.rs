...
//! labeled text inputs, and segmented (radio) toggles.
...
/// A horizontal segmented toggle (radio group), e.g. BIOS | UEFI.
#[allow(dead_code)]
pub fn segmented(f: &mut Frame, area: Rect, options: &[&str], selected: usize) {
    ...
    for (i, opt) in options.iter().enumerate() {
        ...
        let b = Block::default()
            ...
            .border_style(bs);
        let p = Paragraph::new(Line::from(Span::styled(*opt, ts)))
...
pub fn input(f: &mut Frame, area: Rect, label: &str, value: &str, focused: bool, mask: bool) {
    ...
    f.render_widget(
        Paragraph::new(Line::from(vec![
            Span::styled(format!(" {shown}{cursor}"), if focused { theme::normal() } else { theme::mute() }),
        ]))
        .block(b),
        rows[1],
    );
