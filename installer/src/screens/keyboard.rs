...
fn is_nonlatin(code: &str) -> bool {
    let c = code.to_lowercase();
    matches!(c.as_str(), "ua" | "by" | "bg" | "gr" | "mk" | "rs" | "ge" | "am" | "il")
        || c.starts_with("ua-")
        || c.starts_with("by-")
        || c.starts_with("bg-")
        || c.starts_with("gr-")
...
fn keymaps() -> &'static Vec<String> {
    ...
    K.get_or_init(|| {
        ...
        let common = [
            "de", "fr", "es", "it", "pl", "cz", "pt", "nl", "se", "fi", "no",
            "dk", "be", "ch", "at", "hu", "ro", "sk", "si", "hr", "gr", "tr",
            "bg", "lt", "lv", "ee", "by", "ca", "br", "la", "is", "ie",
        ];
        ...
        let raw = crate::system::runner::capture("localectl", &["list-keymaps"])
            .unwrap_or_default();
...
pub fn draw(f: &mut Frame, app: &mut App, area: Rect) {
    ...
    f.render_widget(
        Paragraph::new(Line::from(Span::styled(t(app.lang, "kb.hint"), theme::dim()))),
        rows[0],
    );
    ...
    let filter = Paragraph::new(Line::from(vec![
        ...
        Span::styled(
            if app.kb_query.is_empty() { t(app.lang, "kb.filter") } else { app.kb_query.clone() },
            if app.kb_query.is_empty() { theme::mute() } else { theme::normal() },
        ),
    ...
    widgets::action_row(f, rows[3], &t(app.lang, "app.back"), &t(app.lang, "app.next"), app.can_advance);
}
