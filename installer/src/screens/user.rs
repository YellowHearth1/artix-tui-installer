...
pub fn draw(f: &mut Frame, app: &mut App, area: Rect) {
    ...
    let mode_line = Line::from(vec![
        ...
        Span::styled(
            ...
            if cur == Field::Mode { theme::selected() } else { theme::normal() },
        ),
    ]);
    f.render_widget(Paragraph::new(mode_line).block(theme::box_rounded()), rows[0]);
    ...
    for fld in flds.iter().skip(1) {
        ...
        match fld {
            Field::Hostname => widgets::input(
                f, rows[ri], &t(app.lang, "user.hostname"),
                &app.config.hostname, focused, false,
            ),
            Field::Username => widgets::input(
                f, rows[ri], &t(app.lang, "user.username"),
                &app.config.username, focused, false,
            ),
            Field::UserPass => widgets::input(
                f, rows[ri], &t(app.lang, "user.password"),
                &app.config.user_password, focused, true,
            ),
            Field::UserConfirm => widgets::input(
                f, rows[ri], &t(app.lang, "user.confirm"),
                &app.user_confirm, focused, true,
            ),
            Field::RootPass => widgets::input(
                f, rows[ri], &t(app.lang, "user.root_password"),
                &app.config.root_password, focused, true,
            ),
            Field::RootConfirm => widgets::input(
                f, rows[ri], &t(app.lang, "user.root_confirm"),
                &app.root_confirm, focused, true,
            ),
            Field::Mode => {}
        }
        ri += 1;
    ...
    widgets::action_row(
        f, rows[act_idx],
        &t(app.lang, "app.back"), &t(app.lang, "app.next"), ok,
    );
}
...
pub fn handle_key(app: &mut App, key: KeyEvent) {
    ...
    if cur == Field::Mode {
        ...
        let idx = AccountMode::ALL.iter().position(|m| *m == mode(app)).unwrap_or(0);
...
pub fn footer_hint(app: &App) -> String {
    ...
    let flds = fields(app);
    let cur = flds.get(app.user_focus.min(flds.len().saturating_sub(1))).copied();
    match cur {
