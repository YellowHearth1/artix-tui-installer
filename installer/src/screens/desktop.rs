...
pub fn draw(f: &mut Frame, app: &mut App, area: Rect) {
    ...
    let info_lines = {
        ...
        // A desktop that ships BOTH sessions gets a plain "X11/Wayland" line,
        // not a chooser. Plasma and LXQt install both from the same packages
        // and the session is picked at the LOGIN screen, per boot — so offering
        // ←/→ here promised a decision the installer cannot make and does not
        // make: `config.session` was never read by the install plan at all.
        // The code admitted as much ("cosmetic: all of a desktop's sessions are
        // installed"), while the UI still looked like a switch.
        if !d.session_tag().is_empty() {
            let both = d.supports_wayland() && d.supports_x11();
            let mut line = vec![
                Span::styled(
                    format!("  {}: ", t(app.lang, "de.session_label")),
                    theme::dim(),
                ),
                Span::styled(d.session_tag(), theme::normal()),
            ];
            if both {
                line.push(Span::styled(
                    format!("   ({})", t(app.lang, "de.session_both")),
                    theme::dim(),
                ));
            }
...
pub fn handle_key(app: &mut App, key: KeyEvent) {
    ...
    match key.code {
        ...
        // ←/→ used to flip the session for a desktop that ships both. It is
        // gone: the choice was never real. Plasma and LXQt install their X11
        // AND Wayland sessions from the same packages, the session is picked at
        // the login screen on every boot, and `config.session` was read by
        // nothing in the install plan. A control that changes no outcome is
        // worse than no control — it invites the user to deliberate over it.
...
mod tests {
    ...
    /// A desktop that ships both sessions is DESCRIBED, not made a question.
    ///
    /// Plasma and LXQt install their X11 and Wayland sessions from the same
    /// packages and the session is chosen at the login screen on every boot.
    /// The old ←/→ toggle looked like a decision but changed nothing:
    /// `config.session` was read by no part of the install plan. A control that
    /// alters no outcome still costs the user thought, so it is gone.
    #[test]
    fn a_dual_session_desktop_offers_no_fake_choice() {
        for d in [Desktop::KdePlasma, Desktop::Lxqt] {
            assert!(d.supports_wayland() && d.supports_x11());
            assert_eq!(
                d.session_tag(),
                "X11/Wayland",
                "{d:?} does not present both sessions as one"
            );
        }
        // Pressing ←/→ on such a desktop must leave the whole config untouched.
        let mut app = App::new();
        app.cursor = Desktop::ALL
            .iter()
            .position(|d| *d == Desktop::KdePlasma)
            .expect("Plasma is not offered");
        app.de_focus = 0;
        let before = app.config.session.clone();
        for code in [KeyCode::Left, KeyCode::Right] {
            handle_key(&mut app, KeyEvent::new(code, KeyModifiers::NONE));
        }
        assert_eq!(
            app.config.session, before,
            "←/→ still flips a session that nothing downstream reads"
        );
    }

    /// The X11-with-seatd workaround follows what the desktop CAN run, never a
    /// session flag — otherwise removing the toggle would have silently dropped
    /// the rootful-Xorg fix for anyone who leaves Plasma on its Wayland default
    /// and then logs into X11.
    #[test]
    fn the_seat_defaults_do_not_depend_on_the_removed_toggle() {
        let mut app = App::new();
        app.config.desktops = vec![format!("{:?}", Desktop::KdePlasma)];
        apply_desktop_defaults(&mut app, Desktop::KdePlasma);
        assert!(
            Desktop::KdePlasma.supports_x11(),
            "the Xwrapper fix keys off supports_x11(), which must stay true here"
        );
    }
}
