...
// types a folder name. Bases: skip / under the user's home / under /mnt / a
// full custom path. (No more guessed names like /mnt/windows.)
const BASES: &[&str] = &["", "home", "mnt", "custom"];
    ...
    for d in disk::disks_list() {
        ...
        if pof.is_empty() {
            out.push(Cand {
                dev: d.path.clone(),
                size: d.size.clone(),
                info: if d.model.is_empty() { "disk".into() } else { d.model.clone() },
                format: true,
                whole_disk: true,
                fs_fixed: String::new(),
            });
...
fn cur_mp(app: &App, dev: &str) -> String {
    entry(app, dev).map(|e| e.mountpoint.clone()).unwrap_or_default()
}
fn cur_fs(app: &App, c: &Cand) -> String {
    entry(app, &c.dev)
        .map(|e| e.fs.clone())
        .unwrap_or_else(|| if c.format { "ext4".into() } else { c.fs_fixed.clone() })
}
...
fn fs_pills(app: &App, c: &Cand) -> (Vec<String>, usize) {
    if c.whole_disk {
        ...
        let sel = if !will_format(app, c) {
            ...
            let cur = cur_fs(app, c);
            FS_OPTS.iter().position(|f| *f == cur).map(|p| p + 1).unwrap_or(1)
        };
        (opts, sel)
    }
}
...
fn cur_base(app: &App, dev: &str) -> String {
    entry(app, dev).map(|e| e.mount_base.clone()).unwrap_or_default()
}
...
fn in_use(app: &App, dev: &str) -> bool {
    entry(app, dev).map(|e| !e.mountpoint.is_empty()).unwrap_or(false)
}
fn taken_by_other(app: &App, dev: &str, mp: &str) -> bool {
    !mp.is_empty()
        && app
            .config
            .extra_disks
            .iter()
            .any(|e| e.disk != dev && e.mountpoint == mp)
}
...
fn entry_mut<'a>(app: &'a mut App, c: &Cand) -> &'a mut ExtraDisk {
    if !app.config.extra_disks.iter().any(|e| e.disk == c.dev) {
        let fs = if c.format { "ext4".to_string() } else { c.fs_fixed.clone() };
...
pub fn draw(f: &mut Frame, app: &mut App, area: Rect) {
    ...
    f.render_widget(
        Paragraph::new(vec![
            Line::from(Span::styled(t(app.lang, "storage.intro1"), theme::heading())),
    ...
    if cands.is_empty() {
        ...
        for (i, c) in cands.iter().enumerate() {
            ...
            let title_style = if is_sel { theme::title() } else { theme::normal() };
            ...
            let mut fs_spans = vec![
                Span::styled(if fs_here { "   \u{203a} " } else { "     " }, theme::accent()),
                Span::styled(format!("{}  ", t(app.lang, "storage.lbl_format")), theme::dim()),
            ];
            ...
            let mut mp_spans = vec![
                Span::styled(if mp_here { "   \u{203a} " } else { "     " }, theme::accent()),
                Span::styled(format!("{}  ", t(app.lang, "storage.lbl_mount")), theme::dim()),
            ];
            ...
            if base_needs_name(&base) {
                let custom = base == "custom";
                let name = entry(app, &c.dev).map(|e| e.mount_name.clone()).unwrap_or_default();
                let label = if custom {
                ...
                if !preview.is_empty() {
                    nspans.push(Span::styled(format!("    \u{2192} {}", preview), theme::mute()));
                }
            ...
            if will_format(app, c) {
                ...
                let state = t(app.lang, if on { "storage.enc_on" } else { "storage.enc_off" });
                ...
                lines.push(Line::from(vec![
                    Span::styled(if enc_here { "   \u{203a} " } else { "     " }, theme::accent()),
                    Span::styled(format!("{}  ", t(app.lang, "storage.lbl_encrypt")), theme::dim()),
...
fn draw_storage_opts_modal(f: &mut Frame, app: &App, area: Rect) {
    ...
    let block = Block::default()
        ...
        .title(format!(" {} \u{2014} {} ", t(app.lang, "disk.fsopt_modal_title"), c.dev))
...
pub fn handle_key(app: &mut App, key: KeyEvent) {
    ...
    if app.storage_opts_modal_open {
        ...
        match key.code {
            ...
            KeyCode::Char(' ') => {
                if let Some(id) = opts.get(app.storage_opt_cursor.min(opts.len().saturating_sub(1))).copied() {
    ...
    match key.code {
        ...
        KeyCode::Char('j') if !editing_name => {
            app.storage_cursor = (cur + 1).min(rlist.len() - 1)
        }
