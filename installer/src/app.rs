...
/// Firmware/boot mode for partitioning (spec step 7).
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum BootMode {
    /// BIOS / legacy — used for VM testing. Creates a BIOS boot partition.
    Bios,
    /// UEFI — recommended for real hardware. Creates an ESP.
    Uefi,
}
...
impl Desktop {
    ...
    pub fn packages(self) -> &'static [&'static str] {
        match self {
            ...
            Desktop::Gnome => &["gnome"],
            // XFCE: group + goodies, as the Artix guide recommends.
            Desktop::Xfce4 => &["xfce4", "xfce4-goodies"],
            // Cinnamon is shipped as a single package in galaxy (not a group);
            // a file manager (nemo) makes it usable out of the box.
            Desktop::Cinnamon => &["cinnamon", "nemo", "metacity", "lightdm", "lightdm-gtk-greeter"],
    ...
    /// Does the *effective* session (given the user's pick) run on Wayland?
    /// Used to decide seat-manager packages/services. A desktop that only
    /// supports one session ignores the stored pick.
    pub fn session_is_wayland(self, chosen: &str) -> bool {
        match (self.supports_wayland(), self.supports_x11()) {
            (true, false) => true,                 // Wayland-only
            (false, true) => false,                // X11-only
            (true, true) => chosen == "wayland",   // both → honor the choice
    ...
    /// X11 desktops need a real X server. On Artix the default is now XLibre,
    /// which can be flaky with some software, so we explicitly install the
    /// genuine X.Org server plus the common X utility apps for any X11 desktop.
    /// Wayland desktops don't need this (only Xwayland, added per-desktop).
    /// X11 desktops need a real X server. On Artix the default is now XLibre,
    /// which can be flaky with some software, so we explicitly install the
    /// genuine X.Org server plus the common X utility apps when the effective
    /// session is X11. Wayland sessions don't need it (only Xwayland).
    /// Whether we should pre-install an X server (as named targets, in the
    /// first transaction). True for ANY graphical desktop — not just X11 — for
    /// two reasons: X11 sessions obviously need it, and even Wayland sessions
    /// pull SDDM, whose `x11win-server` dependency would otherwise resolve to
    /// XLibre by default and conflict. Pre-installing xorg-server (named)
    /// satisfies that dependency cleanly. `None` desktop needs no X server.
    pub fn needs_xorg(self, _chosen_session: &str) -> bool {
        !matches!(self, Desktop::None)
    }

    /// Whether this is a Wayland session (so we only need Xwayland for X11 apps,
    /// not the full xf86 driver stack).
    pub fn wayland_session(self, chosen_session: &str) -> bool {
        self.session_is_wayland(chosen_session)
    }
...
pub const DINIT_SERVICES: &[&str] = &[
    ...
    "ntpd",            // time sync
    "nftables",        // firewall (loads our /etc/nftables.conf)
...
pub struct InstallConfig {
    ...
    pub desktop: String,        // serialized Desktop
...
pub struct App {
    ...
    pub pkg_rx: Option<crossbeam_channel::Receiver<Result<Vec<crate::system::packages::Pkg>, String>>>,
    ...
    pub aur_rx: Option<crossbeam_channel::Receiver<Result<Vec<crate::system::packages::Pkg>, String>>>,
