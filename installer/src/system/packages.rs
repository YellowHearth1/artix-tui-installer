...
pub fn popular() -> Vec<Pkg> {
    const ITEMS: &[(&str, &str)] = &[
        ...
        ("octopi", "Graphical package manager (pacman frontend)"),
    ...
    ITEMS
        ...
        .map(|(n, d)| Pkg { name: n.to_string(), desc: d.to_string() })
        .collect()
}

/// Fetch and cache the full repo list (name only) for offline browsing.
pub fn list() -> &'static Vec<String> {
    static P: OnceLock<Vec<String>> = OnceLock::new();
    P.get_or_init(|| {
        let raw = super::runner::capture(
            "pacman",
            &["-Sl", "system", "world", "galaxy", "lib32", "extra", "multilib"],
        )
        .unwrap_or_default();
        let mut v: Vec<String> = raw
            .lines()
            .filter_map(|l| l.split_whitespace().nth(1).map(|s| s.to_string()))
            .filter(|n| !is_other_init(n))
            .collect();
        v.sort();
        v.dedup();
        v
...
pub fn search(query: &str) -> Result<Vec<Pkg>, String> {
    ...
    while let Some(head) = lines.next() {
        ...
        let name = after_slash.split_whitespace().next().unwrap_or("").to_string();
...
pub fn aur_popular() -> Vec<Pkg> {
    [
        ...
        ("asusctl-nosystemd-dinit", "ASUS laptop control (asusctl) for dinit, no systemd"),
        ("visual-studio-code-bin", "Microsoft's VS Code editor (official binary)"),
        ("spotify", "Spotify music streaming client"),
        ("vesktop-bin", "Discord client with Vencord + better Linux screenshare"),
        ("librewolf-bin", "Privacy-oriented Firefox fork (binary)"),
        ("obsidian", "Markdown knowledge base / note-taking app"),
        ("heroic-games-launcher-bin", "Launcher for Epic Games, GOG and Amazon Prime"),
        ("protonup-qt", "Install & manage Proton-GE / Wine-GE for Steam & Lutris"),
        ("grayjay-bin", "FUTO Grayjay: multi-platform video/streaming app"),
        ("spicetify-cli", "Command-line tool to customize the Spotify client"),
        ("pinnacle-comp", "Smithay Wayland compositor, AwesomeWM-like (Lua/Rust)"),
        ...
        ("jellyfin-desktop", "Native desktop client for Jellyfin media server"),
        ("jellyfin-desktop-git", "Jellyfin desktop client, built from git"),
        ("flameshot-git", "Powerful screenshot tool with annotation (git)"),
        ("appimagelauncher", "Integrate, manage and run AppImage apps"),
        ("authme-bin", "Desktop 2FA/TOTP authenticator (binary)"),
        ("chatterino2-7tv-native-git", "Chatterino Twitch chat client with 7TV support (git)"),
        ("dracula-gtk-theme-full", "Dracula dark theme for GTK (full variant)"),
        ("geany-git", "Geany: lightweight GTK IDE, built from git"),
        ("grim-git", "Grim: screenshot utility for Wayland compositors (git)"),
        ("neo-candy-icons", "Neon Candy SVG icon theme"),
        ("obs-multi-rtmp", "OBS Studio plugin for streaming to multiple RTMP targets"),
        ("pokesay-bin", "Pokémon-themed cowsay (binary)"),
        ("satty-git", "Satty: screenshot annotation tool for Wayland (git)"),
        ("steamguard-cli-bin", "Command-line Steam Guard 2FA generator (binary)"),
        ("ttf-apple-emoji", "Apple color emoji font"),
        ("ttf-hanazono", "Hanazono: free CJK kanji font"),
        ("ttf-joypixels", "JoyPixels color emoji font"),
        ("wireguard-dkms", "WireGuard kernel module via DKMS (for older kernels)"),
        ("xone-dkms-git", "xone: Xbox One/Series controller driver, DKMS (git)"),
        ("xone-dongle-firmware", "Firmware for the Xbox wireless dongle (xone)"),
    ]
    .iter()
    .map(|(n, d)| Pkg { name: n.to_string(), desc: d.to_string() })
    .collect()
