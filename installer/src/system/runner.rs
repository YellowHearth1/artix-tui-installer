...
/// Spawn `program args...` and stream its output into a channel. Returns the
/// receiver immediately; the install screen drains it on each tick so the log
/// scrollback updates live. Non-blocking by design.
pub fn spawn(program: &str, args: &[&str]) -> Receiver<LogLine> {
    spawn_with_mode(program, args, false)
}
...
pub fn spawn_pty(program: &str, args: &[&str]) -> (Receiver<LogLine>, PtyWriter) {
    ...
    let pair = match pty_system.openpty(PtySize { rows: 40, cols: 120, pixel_width: 0, pixel_height: 0 }) {
        ...
        Err(e) => {
            ...
            return (rx, PtyWriter { inner: std::sync::Arc::new(std::sync::Mutex::new(sink)) });
        }
    };
    ...
    if std::env::var_os("PATH").is_none() {
        cmd.env("PATH", "/usr/local/sbin:/usr/local/bin:/usr/bin:/usr/sbin:/sbin:/bin");
    }
    ...
    let pty_writer = PtyWriter { inner: std::sync::Arc::new(std::sync::Mutex::new(writer)) };
    ...
    std::thread::spawn(move || {
        ...
        let result = match child.wait() {
            ...
            Ok(status) => Err(format!("{} exited with code {}", prog_name, status.exit_code())),
