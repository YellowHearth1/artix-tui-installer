...
pub fn list() -> Result<Vec<Disk>, String> {
    let json = super::runner::capture(
        "lsblk",
        &["--json", "-o", "NAME,SIZE,TYPE,MODEL,RM", "-b", "--nodeps"],
    )?;
    // We asked for -b (bytes) for filtering but want human size too; re-query
    // human-readable separately would be wasteful, so just re-run without -b.
    ...
    let _ = json; // reserved for future size-threshold filtering
...
pub fn list_partitions() -> Result<Vec<Partition>, String> {
    let json = super::runner::capture(
        "lsblk",
        &["--json", "-o", "NAME,SIZE,TYPE,FSTYPE,LABEL"],
    )?;
...
pub(crate) fn part(disk: &str, n: u32) -> String {
    let needs_p = disk
        .rsplit('/')
        .next()
        .map(|n| n.chars().last().map(|c| c.is_ascii_digit()).unwrap_or(false))
        .unwrap_or(false);
    if needs_p {
        format!("{disk}p{n}")
    } else {
...
pub fn build_plan(
    ...
    if uefi {
        ...
        plan.push(act(
            "sgdisk",
            &["-n", &format!("{idx}:0:+512M"), "-t", &format!("{idx}:ef00"), "-c", &format!("{idx}:EFI"), disk],
        ));
        esp_n = Some(idx);
        idx += 1;
    } else {
        ...
        plan.push(act(
            "sgdisk",
            &["-n", &format!("{idx}:0:+1M"), "-t", &format!("{idx}:ef02"), "-c", &format!("{idx}:BIOSBOOT"), disk],
        ));
        esp_n = Some(idx);
        idx += 1;
    }
    ...
    if enc_boot {
        plan.push(act(
            "sgdisk",
            &["-n", &format!("{idx}:0:+1G"), "-t", &format!("{idx}:8300"), "-c", &format!("{idx}:BOOT"), disk],
        ));
        bootpart_n = Some(idx);
        idx += 1;
    } else {
    ...
    if swap {
        plan.push(act(
            "sgdisk",
            &["-n", &format!("{idx}:0:+{swap_gib}G"), "-t", &format!("{idx}:8200"), "-c", &format!("{idx}:SWAP"), disk],
        ));
        swap_n = Some(idx);
        idx += 1;
    } else {
    ...
    plan.push(act(
        "sgdisk",
        &["-n", &format!("{idx}:0:0"), "-t", &format!("{idx}:8300"), "-c", &format!("{idx}:ROOT"), disk],
    ));
    root_n = idx;
    ...
    let common: Vec<&str> = if mount_noatime { vec!["noatime"] } else { vec![] };
    ...
    if root_fs == "btrfs" && btrfs_subvolumes {
        ...
        for sv in ["@", "@home", "@snapshots", "@log", "@cache"] {
            plan.push(act("btrfs", &["subvolume", "create", &format!("/mnt/{sv}")]));
        }
    ...
    if root_fs == "btrfs" && btrfs_subvolumes {
        plan.push(act(
            "mkdir",
            &["-p", "/mnt/home", "/mnt/.snapshots", "/mnt/var/log", "/mnt/var/cache"],
        ));
        for (sv, target) in [
            ("@home", "/mnt/home"),
