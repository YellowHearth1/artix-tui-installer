...
pub(crate) const XORG_PACKAGES: &[&str] = &[
    ...
    "xorg-xdpyinfo",
    "xorg-setxkbmap",
    // The X11 layout database itself. It arrives as a dependency of the Xorg
    // stack anyway, but a graphical session is unusable without it and a
    // headless-then-desktop install must not depend on that chain holding —
    // so it is named outright.
    "xkeyboard-config",
    // Console keymaps (us.map among them) and loadkeys. Same reasoning: the
    // console keymap written by the installer is worthless if kbd is absent.
    "kbd",
    "xorg-fonts-misc",
