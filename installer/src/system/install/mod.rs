...
pub fn build_plan(app: &App) -> Vec<Action> {
    ...
    //    installer speaks. ter-116n was wrong for this: reading its PSF Unicode
    //    table shows all twelve Spanish accented letters and NOT ONE Cyrillic
    //    one — so a Ukrainian install, the project's own first language, came up
    ...
    // on such consoles Backspace "does nothing". The wrapper pins keycode 14
    // back to Delete; KEYMAP= accepts an absolute path, and mkinitcpio's
    // `keymap` hook compiles the same file into the initramfs, so the LUKS
    // prompt gets the fix too.
    //
    // The include carries an ABSOLUTE PATH, resolved on the target at install
    // time. It used to name the map bare — `include "ua-utf"` — and loadkeys
    // cannot resolve that: it looks includes up only in the keymaps' `include/`
    // directories (.inc files), never among the .map files. So EVERY install
    // shipped a keymap file that fails to parse, `loadkeys -q -u` returned an
    // error, dinit's vconsole helper swallowed it with `|| :`, and the console
    // silently stayed on the kernel default — taking the Backspace fix and the
    // LUKS prompt's layout down with it. Verified with loadkeys itself:
    // `include "ua-utf"` → "cannot open include file"; the absolute path → ok.
    //
    // The name is resolved by searching, not by assuming a path, because the
    // layout of /usr/share/kbd/keymaps differs between architectures and the
    // files may or may not be gzipped. Two aliases are applied first: the
    // keyboard list stores X11 codes, and for these two the console disagrees —
    // X11 "gb" is kbd "uk" (British), X11 "latam" is kbd "la-latin1". Without
    // that translation an English install asked for `gb`, which does not exist
    // as a console keymap at all. If nothing matches, it falls back to `us`
    ...
    //     on the summary screen, and were never written anywhere — a graphical
    //     session fell back to whatever xkeyboard-config defaults to.
    //
    //     Written twice on purpose, because nothing reads both:
    //       * /etc/X11/xorg.conf.d/00-keyboard.conf — X11 and Xwayland;
    //       * XKB_DEFAULT_* in /etc/environment — wlroots-based Wayland
    //         compositors (Sway, MangoWC, Pinnacle), which never look at
    //         xorg.conf.d. On a systemd distro localectl would write the first
    //         and the compositor would ask logind; there is no logind here.
    //
    //     `us` is force-appended when the user did not pick it. Native Steam
    //     needs a us group present — and it is not alone; several games and
    ...
    plan.push(write_target_file(
        ...
        &format!(
            "# Managed by the Artix installer.\n\
             ...
             \tMatchIsKeyboard \"on\"\n\
             \tOption \"XkbLayout\" \"{layout_csv}\"\n\
             \tOption \"XkbOptions\" \"grp:alt_shift_toggle\"\n\
             EndSection\n"
        ),
    ));
    plan.push(chroot(&format!(
        "grep -q '^XKB_DEFAULT_LAYOUT=' /etc/environment 2>/dev/null \
...
mod tests {
    ...
    /// a mirror is only useful near you, so someone who moves needs to comment
    /// out one block and uncomment another. An earlier version sorted every
    /// surviving mirror into one flat, speed-ordered list and threw the country
    /// sections away — leaving a file that cannot be edited by hand, because
    /// nothing on a `Server =` line says where the machine is.
    ///
    /// Mirrors that fail the health check are COMMENTED OUT, never deleted:
    /// one that is down today may be the closest one after a move.
    #[test]
    fn the_rebuilt_mirrorlist_keeps_country_headings() {
        let sh = MIRROR_OPTIMIZE_SCRIPT;
        assert!(
            sh.contains("collect_with_country"),
        ...
        assert!(
            ...
            "failing mirrors are not commented out"
        );
        // And the file must say how to use the blocks, or the structure is a
        // detail nobody discovers.
        assert!(
            sh.contains("Moved to another country?"),
            "the rebuilt list does not explain how to switch country"
        );
    }

    /// The keyboard layouts the user picked actually REACH the installed system.
    ///
    /// They used to reach nothing at all: `xkb_layouts` was read by the keyboard
    /// screen, echoed on the summary, and then dropped. Nothing in the plan ever
    /// mentioned it, so a graphical session came up on whatever xkeyboard-config
    /// defaults to and the whole step was decoration.
    ///
    ...
    fn the_chosen_keyboard_layouts_reach_the_installed_system() {
        let mut a = install_app();
        a.config.xkb_layouts = vec!["ua".into(), "gb".into()];
        let t = plan_text(&build_plan(&a));
        assert!(
            t.contains("00-keyboard.conf"),
            "no X11 keyboard config is written — the keyboard step is decorative"
        );
        assert!(
            t.contains("XKB_DEFAULT_LAYOUT"),
    ...
    /// `us` is always among the layouts, even when the user never picks it.
    ///
    /// Native Steam needs a us group present, and it is not alone — several
    /// games and Electron apps read raw keycodes against us. It is appended
    /// LAST so it never displaces the user's own primary layout, which still
    /// has to be the one the console and the LUKS prompt agree with.
    #[test]
    fn us_is_always_available_because_steam_needs_it() {
        for picked in [vec!["ua".to_string()], vec![], vec!["us".to_string()]] {
            let mut a = install_app();
            a.config.xkb_layouts = picked.clone();
            let t = plan_text(&build_plan(&a));
            assert!(
                t.contains("XkbLayout"),
                "{picked:?}: no layout written at all"
            );
            let line = t
                .lines()
                .find(|l| l.contains("XKB_DEFAULT_LAYOUT="))
                .unwrap_or_default()
                .to_string();
            assert!(
                line.contains("us"),
                "{picked:?}: us is missing — native Steam breaks: {line}"
            );
            // Present exactly once, and never ahead of a deliberate choice.
            if let Some(first) = picked.first() {
                assert!(
                    line.contains(&format!("XKB_DEFAULT_LAYOUT={first}")),
                    "{picked:?}: us displaced the user's primary layout: {line}"
                );
            }
        }
    }
    ...
    /// actually ship on the image, and the result contradicted the comment that
    /// had sat here for versions: ter-116n has all twelve Spanish accented
    /// letters and not one Cyrillic letter, so a Ukrainian install — the
    /// project's first language — booted into a console that could not draw its
    /// own interface. UniCyr_8x16, which the live launcher loaded, is the exact
    /// mirror image: Cyrillic yes, "Español" rendered as "Espanol".
    ///
    /// LatArCyrHeb-16+ covers both and comes from `kbd`, which is in the base
    /// package set — so it is present even on an install without terminus-font.
    #[test]
    ...
    /// The console keymap is written as a path loadkeys can actually open.
    ///
    /// It used to name the map bare — `include "ua-utf"` — and loadkeys cannot
    /// resolve that: includes are looked up only in the keymaps' `include/`
    /// directories (.inc files), never among the .map files. Every install
    /// therefore shipped a keymap that fails to parse; dinit's vconsole helper
    /// runs `loadkeys -q -u "$KEYMAP" || :` and swallowed the error, so the
    /// console silently stayed on the kernel default — and the same file goes
    /// into the initramfs, so the LUKS passphrase prompt lost its layout too.
    /// Confirmed against loadkeys itself, for every language, including the
    /// gb→uk and latam→la-latin1 aliases and the us fallback.
    #[test]
    fn the_console_keymap_is_written_as_a_resolvable_path() {
        let t = plan_text(&build_plan(&install_app()));
        assert!(
            t.contains("find /usr/share/kbd/keymaps"),
            "the keymap path is assumed rather than resolved on the target"
        );
        assert!(
            !t.contains("include \\\"$km\\\""),
            "the include still names the map bare — loadkeys cannot open that"
        );
        // The console and X11 disagree on these two names; without the aliases
        // an English install asks for `gb`, which is not a console keymap.
        assert!(
            t.contains("gb) km=uk") && t.contains("latam) km=la-latin1"),
            "the X11→console keymap aliases are missing"
        );
        // Never write a file that cannot load.
        assert!(
            t.contains("us.map*"),
            "no fallback: an unknown code would produce an unloadable keymap"
        );
    }
