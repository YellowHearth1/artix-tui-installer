...
Покладіть скомпільований бінарник у **`root-overlay/usr/bin/`** профілю, тоді
виконайте `sudo buildiso -p <профіль>`. Саме `root-overlay`, а не `live-overlay`:
`buildiso` копіює в кореневу ФС образу лише перший, а до другого доходить лише
тоді, коли профіль будує окремий livefs. Покладений не туди бінарник просто не
потрапляє в ISO — і збірка про це не скаржиться.
...
Повний прохід майстра — усі **15 кроків**. Інтерфейс доступний українською, англійською та іспанською; знімки нижче зроблено
українською.
...
**Крок 3/15 — Мережа.** Пропустити (дротове з'єднання) або просканувати Wi-Fi: вибір адаптера, мережі та введення пароля.
...
**Крок 6/15 — Графічне оточення.** Мультивибір оточень і вибір екрана входу. Оточення, що вміють обидві сесії (Plasma, LXQt), ставлять і X11, і Wayland — сесія обирається при вході.
...
**Крок 12/15 — Облікові записи.** Ім'я комп'ютера, користувач і паролі; режим облікових записів.
